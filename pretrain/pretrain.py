...
class MolCLR(object):
    ...
    def _get_device(self):
        ...
        gpu_idx = self.config.get('gpu', '0')

        if gpu_idx is None or str(gpu_idx).lower() == 'cpu':
            device = torch.device('cpu')
        else:
            if str(gpu_idx).isdigit():
                device = torch.device(f'cuda:{gpu_idx}')
            else:
                raise ValueError(f"Invalid GPU specification: {gpu_idx}")
    ...
    def _load_pre_trained_weights(self, model, device=None):
        try:
